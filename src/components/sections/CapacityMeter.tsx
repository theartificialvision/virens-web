 ...
 * como algo que se llena). Dos cosas suben a la vez, con la misma curva:
 *
 ...
 *
 * (El filete de proporción bajo la cifra se retiró el mismo día a petición
 * del cliente.)
 *
 * El texto real va en `sr-only`; lo que cuenta es `aria-hidden`, para que un
 * lector de pantalla no lea los valores intermedios.
 */
export function CapacityMeter({ item, index }: { item: CapacityItem; index: number }) {
