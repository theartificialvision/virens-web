...
export default function VirensLabsPage() {
  return (
    <>
      {/* 01 */}
      <HeroVideo {...labsHero} accent="labs" />
      ...
      <Section tone="white" rhythm="base">
        <Container>
          <Eyebrow className="text-labs-ink">{labsIntro.eyebrow}</Eyebrow>
          <ul className="mt-10 grid border-l border-t border-hairline sm:grid-cols-3">
            {labsIntro.points.map((p) => (
              <li key={p.index} className="flex min-h-52 flex-col justify-between border-b border-r border-hairline p-7 lg:p-9">
                <span className="text-[length:var(--text-h3)] font-bold leading-none tracking-[-0.03em] text-labs-ink">{p.index}</span>
